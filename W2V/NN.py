...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dz = torch.mm(softmax, W2.t())
    dh = torch.mm(softmax, W2.t())*(z > 0)

    grads['W2'] = torch.mm(h.t(), softmax) / N
    grads['b2'] = torch.sum(softmax) / N
    grads['W1'] = torch.mm(X.t(), dh) / N
